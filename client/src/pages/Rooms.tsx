    ...
    <div className="min-h-screen flex flex-col bg-stone-50 text-stone-900">
      ...
      <section className="relative isolate overflow-hidden bg-black pt-36 pb-24">
        ...
        <div className="relative container mx-auto px-6 lg:px-12 max-w-6xl text-white space-y-8">
          ...
          >
            From sun-graced deluxe suites to expansive family cottages, each
            space is crafted with warmth and intent.
          </motion.p>

          <div className="flex flex-col sm:flex-row gap-4">
            <Button
              size="lg"
              className="rounded-full px-8 py-6 uppercase tracking-[0.4em] bg-amber-400 text-black"
              onClick={handleBookingRedirect}
            >
              Book a Stay
            </Button>
            <Button
              size="lg"
              className="rounded-full border border-zinc-700 bg-zinc-900 text-white px-8 py-6 uppercase tracking-[0.4em]"
              onClick={() => navigate("/")}
            >
              Return Home
            </Button>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-8">
            {highlightTiles.map((tile) => (
              ...
              >
                ...
                <p className="text-sm text-white/80">{tile.copy}</p>
              </motion.div>
            ))}
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {sortedRooms.map((room, index) => (
              <motion.article
                ...
                animate={{ opacity: 1, y: 0 }}
                transition={{ delay: index * 0.05 }}
                className="rounded-3xl overflow-hidden bg-white shadow-lg flex flex-col"
              >
                <div
                  className="h-56 w-full bg-cover bg-center"
                  style={{
                    backgroundImage: `url(${
                      room.image?.[0] ?? fallbackRoomImage
                    })`,
                  }}
                />
        ...
        <Dialog open={detailsOpen} onOpenChange={setDetailsOpen}>
          <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              ...
              <DialogDescription>{selectedRoom.Roomtype}</DialogDescription>
            </DialogHeader>

            {/* IMAGES GRID */}
            <div className="grid grid-cols-2 gap-4 mb-6">
              {selectedRoom.image?.map((img: string, idx: number) => (
                <img
                  key={idx}
                  src={img}
            ...
            <p className="text-sm text-stone-500">
              Capacity: {selectedRoom.capacity} {selectedRoom.occupancyDetails}
            </p>
            ...
            <div className="mt-4">
              ...
              <p className="text-xl font-semibold text-stone-900">
                {currencyFormatter.format(selectedRoom.price)} / night
              </p>
              <p className="text-sm text-stone-500">
